...
function userSample(){
		for(var n in json) {
			var userKey = Math.floor(Math.random()*1000001111);
			localStorage.setItem(userKey, JSON.stringify(json[n]));
			window.location.reload();
		}
	}                
function radioButton(){
		var radio = $('userForm').sex;
                var array = [];
		for(var i=0, j=radio.length; i<j; i++){
			if(radio[i].checked){
				var userSex = radio[i].value;
                array.push(userSex);
				return userSex;
			}
		}
...
function viewUser(){
        displayData("on");
		if(localStorage.length === 0){
			alert("Currently there are no Users to view. We have added some sample information to get you started.");
			userSample();
		}else{
		...
		for(var i=0, x = localStorage.length; i<x; i++){
			...
			travelDates(showData.length[1], myUser);
...
function editUser(){
		...
		var rad = $('userForm').sex;
		for(var i=0; i<rad.length; i++){
			if(rad[i].value == "male" && data.usersSex[1] == "male"){
				rad[i].setAttribute("checked", "checked");
			}else if(rad[i].value == "female" && data.usersSex[1] == "female"){
				rad[i].setAttribute("checked", "checked");
			}
		}
